      ...
      <el-col :span="5">
        <el-form label-position="left">
          <el-form-item label="资源池名称:">
            <el-select class="el-select" v-model="selectedPools" multiple :loading="poolsLoading" placeholder="资源池名"
              @visible-change="poolSelectBoundry" @change="handleSelectedPoolChange" >
...
import { ListPoolRunningMetrics, ListPoolList } from '@/api/resource_pool_running_metrics'
...
export default {
  ...
  created() {
    ...
    this.requestDataOfFirstPool()
  },
  mounted() {
    this.getColumeOptions()
  },
  methods: {
    requestDataOfFirstPool() {
      let queryPoolsListParams = {
        StartTime: this.queryParams.StartTime,
        EndTime: this.queryParams.EndTime,
        TimeType: this.queryParams.TimeType
      }
      ListPoolList(queryPoolsListParams).then(response => {
        if (response.Data != null && response.Data.length > 0) {
          this.selectedPools = [response.Data[0].PoolName]
          this.handleSelect()
        }
      })
    },
    ...
    // 开始或结束节点选择
    poolSelectBoundry(begin) {
      if (begin) {
        this.isSelectingPool = true
        this.fetchAllPools()
      } else {
        // 结束选择时，查询当前选择地址列表的信息
        this.isSelectingPool = false
        this.handleSelect()
      }
    },
    ...
    fetchAllPools() {
      ...
      this.resourcePoolList = []
      ListPoolList(queryPoolsListParams).then(response => {
        if (response.Data != null && response.Data.length > 0) {
          response.Data.forEach(metricRecord => {
            this.resourcePoolList.push(metricRecord.PoolName)
          })
        }
    ...
    handleSelect() {
      ...
      this.queryParams.PoolNames = this.selectedPools
     ...
     *  该场景通过this.isSelectingPool变量跟下拉选择的场景区分
     */
    handleSelectedPoolChange() {
      if (this.isSelectingPool) {
        return
      }
      // 异步执行，metricDataLoading修改会触发界面更新，导致用户删除操作卡顿
      setTimeout(() => {
          this.metricDataLoading = true
          this.deleteDataOfRemovedPool()
          // 更新表格第一页的信息
          this.queryParams.Page = 1
          this.queryParams.Limit = this.PageLimit
          this.updateTableInfo()
    ...
    deleteDataOfRemovedPool() {
      let remainingPoolSet = new Set()
      this.selectedPools.forEach(pool => {
        remainingPoolSet.add(pool)
      })
      let strippedDataAll = []
      for (let index = 0; index < this.metricDataAll.length; index++) {
        if (remainingPoolSet.has(this.metricDataAll[index].PoolName)) {
    ...
    updateTableAndCharts() {
      this.metricDataLoading = true
      ListPoolRunningMetrics(this.queryParams).then(response => {
        // 符合筛选结果的所有信息
