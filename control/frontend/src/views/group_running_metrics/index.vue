      ...
      <el-col :span="5">
        <el-form label-position="left">
          <el-form-item label="群组名:">
            <el-select class="el-select" v-model="selectedGroups" multiple :loading="groupLoading" placeholder="群组名"
              @visible-change="groupSelectBoundry" @change="handleSelectedGroupChange" >
...
import { ListGroupRunningMetrics, ListGroupList } from '@/api/group_running_metrics'
...
export default {
  ...
  created() {
    ...
    this.requestDataOfFirstGroup()
  },
  mounted() {
    this.getColumeOptions()
  },
  methods: {
    requestDataOfFirstGroup() {
      let queryGroupsListParams = {
        StartTime: this.queryParams.StartTime,
        EndTime: this.queryParams.EndTime,
        TimeType: this.queryParams.TimeType
      }
      ListGroupList(queryGroupsListParams).then(response => {
        if (response.Data != null && response.Data.length > 0) {
          this.selectedGroups = [response.Data[0].GroupName]
          this.handleSelect()
        }
      })
    },
      ...
      ListGroupList(queryGroupsListParams).then(response => {
        if (response.Data != null && response.Data.length > 0) {
          response.Data.forEach(metricRecord => {
            this.groupList.push(metricRecord.GroupName)
          })
        }
    ...
    handleSelect() {
      ...
      this.queryParams.GroupNames = this.selectedGroups
     ...
     *  该场景通过this.isSelectingGroup变量跟下拉选择的场景区分
     */
    ...
    deleteDataOfRemovedGroup() {
      let remainingGroupSet = new Set()
      this.selectedGroups.forEach(group => {
        remainingGroupSet.add(group)
      })
      let strippedDataAll = []
      for (let index = 0; index < this.metricDataAll.length; index++) {
        if (remainingGroupSet.has(this.metricDataAll[index].GroupName)) {
    ...
    updateTableAndCharts() {
      this.metricDataLoading = true
      ListGroupRunningMetrics(this.queryParams).then(response => {
        // 符合筛选结果的所有信息
